...
function Get-DocumentCategory {
    # Extension lists mirror exactly the "Supported file types" table in the Litera
    # Compare Command Line spec (page 3) for lcp_auto.exe input.
    # .docm / .dotm / .txt are not in the PDF's table and are excluded for the Auto
    # engine; they remain valid inputs for lcp_main.exe (Word engine) only.
    param([string]$extension)
    $ext = $extension.TrimStart('.').ToLowerInvariant()
    if (@('doc','docx','rtf','pdf','wpd','htm','html') -contains $ext) {
        return 'Document'
...
function Get-EngineSupportedInputExtensions {
    ...
    switch ($Engine) {
        'Auto' {
            # Matches the PDF spec "Supported file types" table exactly.
            # .docm / .dotm / .txt are not listed and are excluded here.
            return @(
                '.doc', '.docx', '.rtf', '.pdf', '.wpd', '.htm', '.html',
                '.xls', '.xlsx', '.xlsm', '.xlsb',
        ...
        'Word' {
            # lcp_main.exe accepts these as input (cross-format pairs such as
            # PDF vs .docx, PDF vs .html, etc. are supported in practice).
            # Note: .wpd / .htm / .html are INPUT-ONLY — the redline output
            # must be .docx, .doc, .rtf, or .pdf (see Get-AllowedOutputFormats).
            return @('.doc', '.docx', '.docm', '.dotm', '.rtf', '.txt', '.pdf', '.wpd', '.htm', '.html')
        }
        'PowerPoint' {
...
function Get-FileDialogFilterForEngine {
    ...
    switch ($Engine) {
        'Word' {
            return "Word / Text / PDF / Web|*.doc;*.docx;*.docm;*.dotm;*.rtf;*.txt;*.pdf;*.wpd;*.htm;*.html|All Files|*.*"
        }
        'PowerPoint' {
        ...
        default {
            return "Supported Compare Files|*.doc;*.docx;*.rtf;*.pdf;*.wpd;*.htm;*.html;*.xls;*.xlsx;*.xlsm;*.xlsb;*.ppt;*.pps;*.pptx;*.pptm;*.ppsx;*.ppsm;*.png;*.bmp;*.jpg;*.jpeg|All Files|*.*"
        }
    }
}
...
function Test-IsWordAdvancedEligiblePath {
    ...
    $eligibleExtensions = @('.doc', '.docx', '.docm', '.dotm', '.txt', '.rtf', '.pdf', '.wpd', '.htm', '.html')
    ...
    switch ($Engine) {
        'Auto' {
            return @{
                ...
                SupportedOutputLabel = "Documents: .docx/.doc/.rtf/.pdf; Spreadsheets: .xlsm/.xlsx/.xlsb/.xls; Presentations: .pptm/.pptx/.ppt/.ppsm/.ppsx/.pps; Images: .png/.bmp/.jpg/.jpeg"
            }
        }
        'Word' {
            return @{
        ...
        'PowerPoint' {
            return @{
                ...
                SupportsVisible = $true
                SupportsRedlineOnly = $true
                SupportsTrackChanges = $true   # PDF: lcp_ppt.exe uses same options as lcp_main.exe
                SupportsWordPptAdvanced = $true
                SupportsExcelAdvanced = $false
                SupportedOutputLabel = ".pptm/.pptx/.ppt/.ppsm/.ppsx/.pps"
            }
        }
        'Excel' {
            return @{
...
function Get-AvailableOutputFormats {
    ...
    if ($Engine -eq 'Auto') {
        ...
        return @('.docx', '.doc', '.rtf', '.pdf', '.xlsm', '.xlsx', '.xlsb', '.xls', '.pptm', '.pptx', '.ppt', '.ppsm', '.ppsx', '.pps', '.png', '.bmp', '.jpg', '.jpeg')
    }
...
function Get-AllowedOutputFormats {
    ...
    if ($engine -eq 'Auto') {
        switch ($category) {
            ...
            'Document' { return @('.docx', '.doc', '.rtf', '.pdf') }
...
function Validate-ComparisonPair {
    ...
    switch ($Engine) {
        'Word' {
            ...
            return @{ valid = $false; message = "Word engine supports only .doc, .docx, .docm, .dotm, .rtf, .txt, .pdf, .wpd, .htm, and .html as inputs." }
        }
        'PowerPoint' {
...
function Invoke-BulkCompare {
    ...
    Write-Log "Comparison style: $(if ([string]::IsNullOrWhiteSpace($Style)) { 'Default' } else { $Style })"
    ...
    switch ($Mode) {
        'Single' {
            ...
            foreach ($mFile in $modFiles) {
                ...
                $outputFile = Join-Path -Path $OutDir -ChildPath "$Prefix${origName}_$($mFile.BaseName)$Format"
...
$btnTestExe.Add_Click({
    ...
    # Per the Litera Compare Command Line spec, -h / --help is documented only for
    # lcp_auto.exe. The other four executables have no documented help or no-op switch,
    # so launching them with -h is undefined — they may hang, error, or misinterpret it.
    # For those, verify the file is a loadable Windows binary with a recognised name.
    if ($exeName -ne 'lcp_auto.exe') {
        try {
            Write-Log "Testing executable (file check only - '$exeName' has no documented test switch in the Litera spec): $exePath"
            $versionInfo = [System.Diagnostics.FileVersionInfo]::GetVersionInfo($exePath)
            $knownNames = @('lcp_main.exe', 'lcp_ppt.exe', 'lcx_main.exe', 'lcp_pdfcmp.exe')
            if ($knownNames -contains $exeName) {
                Write-Log "Test EXE: '$exeName' is a recognised Litera Compare executable. Product=$($versionInfo.ProductName) Version=$($versionInfo.FileVersion)."
                [System.Windows.MessageBox]::Show(
                    "The file exists and is a valid Windows executable named '$exeName', a recognised Litera Compare component.`n`nNote: unlike lcp_auto.exe, this executable has no documented help/no-op switch in the Litera spec, so this check confirms the file itself rather than performing a live test launch.`n`nPath: $exePath",
                    "Test: File Verified", [System.Windows.MessageBoxButton]::OK, [System.Windows.MessageBoxImage]::Information)
            } else {
                Write-Log "Test EXE: '$exeName' does not match any known Litera Compare executable name."
                [System.Windows.MessageBox]::Show(
                    "The file is a valid Windows executable, but '$exeName' is not a recognised Litera Compare component name.`nExpected one of: lcp_auto.exe, lcp_main.exe, lcp_ppt.exe, lcx_main.exe, lcp_pdfcmp.exe.`n`nDouble-check you have selected the right file for the chosen engine.",
                    "Test: Unexpected File Name", [System.Windows.MessageBoxButton]::OK, [System.Windows.MessageBoxImage]::Warning)
            }
        } catch {
            Write-Log "Test EXE ERROR: $($_.Exception.Message)"
            [System.Windows.MessageBox]::Show(
                "Failed to read the executable's file information - it may not be a valid Windows binary.`n`nError: $($_.Exception.Message)",
                "Test Failed", [System.Windows.MessageBoxButton]::OK, [System.Windows.MessageBoxImage]::Error)
        }
        return
    }

    # lcp_auto.exe: -h is documented in the spec, safe to launch for a live check.
    try {
        ...
        if (-not $exited) {
            $proc.Kill()
            Write-Log "Test EXE: Process launched but did not exit within 3 s. Terminated test instance."
        } else {
            Write-Log "Test EXE: Process launched and exited with code $($proc.ExitCode)."
        }

        [System.Windows.MessageBox]::Show("Successfully connected to the executable!`n`nPath: $exePath",
            "Test Successful", [System.Windows.MessageBoxButton]::OK, [System.Windows.MessageBoxImage]::Information)
    } catch {
        Write-Log "Test EXE ERROR: $($_.Exception.Message)"
        [System.Windows.MessageBox]::Show("Failed to launch the executable.`n`nError: $($_.Exception.Message)",
            "Test Failed", [System.Windows.MessageBoxButton]::OK, [System.Windows.MessageBoxImage]::Error)
    }
})
